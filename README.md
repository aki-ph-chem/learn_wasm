...
`wasm_bindgen`はWasmとJSを跨ぐ複合構造を扱う方法に対する共通の理解を定義していて、以下を含む。Rustの構造をボックス化し,JSのクラスでポインタをラップして使いやすくしたり、Rust側からJSのobjectのテーブルにインデキシングを作成する。
...
良いデザインの例としては、サイズが大きくて、寿命長いデータはWasm側の線形メモリに持たせて、不透明なハンドラとしてJS側に公開するデザインである。
...
Wasmの線形メモリにおける各要素のサイズが1バイトでの一次元配列を用いて宇宙を表現する。
この配列の各要素は`0`なら死を表し、`1`ならば生を表す。
この一次元配列を用いてサイズが`width`の宇宙を表現するならばこの一次元配列全体における添字は`index(row,column) = row * width + column`で計算ができる。

Wasmに宇宙を表現する構造体`Universe`を定義・実装し、JSに公開するには以下の方法を用いる。
まず、`Universe`に対して`std::fmt::Display`を実装する。

このトレイトが実装されることで`Universe`では`to_string()`メソッドが使えるようになるので、セルたちは文字列としてレンダリングされる。レンダリングを行うには、この文字列をJS側のメモリにコピーして、HTMLの`textContent`として描画する。
まずは、コピーを伴う実装を行うが、後にこのコピーを避けて`canvas`に対してレンダリングを行う実装に書き換える。
